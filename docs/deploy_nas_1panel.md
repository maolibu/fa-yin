...
- **对于云服务器 (VPS) 或熟悉命令行的 NAS 用户**：
  ...
  cd /opt/fa-yin
  ```
  此时您的主目录就是 `/opt/fa-yin`。您会看到里面包含了 `docker-compose.yml` 等文件。

- **对于使用可视化面板的用户（1Panel / NAS 网页端）**：
  1. 在本地电脑下载项目源码 ZIP 压缩包并解压。
  2. 登录 1Panel 的“文件”菜单，或 NAS 的“文件管理”。
  3. 在您喜欢的位置（比如 `/volume1/docker/fa-yin`）创建一个文件夹。
...
2. **解压数据**：在您的电脑上将这个压缩包解压，会得到一个名为 `CBETA` 的文件夹（内含 `XML/`、`toc/` 等子目录，共几万个文件）。
3. **上传至服务器**：
...
用文本编辑器打开项目主目录下的 `docker-compose.yml` 文件。**您只需要修改一行**——把 CBETA 数据的路径填进去。

找到 `volumes` 部分，将第一行的 `/path/to/your/cbeta` 替换为您实际存放 CBETA 数据的路径：

```yaml
    volumes:
      # ⚠️ 将冒号左边替换为您的 CBETA 数据实际路径
...
例如，如果您按第二步将 CBETA 放在了项目里的 `data/raw/cbeta`，就改为 `./data/raw/cbeta:/app/data/raw/cbeta`。如果放在 NAS 另一块硬盘上，就填绝对路径如 `/mnt/disk2/cbeta:/app/data/raw/cbeta`。
...
程序必须在容器后台把您放进去的几万份 CBETA XML 文件扫描一遍，并构建出一个能支撑全文毫秒级检索的 SQLite 数据库文件（这也是本项目最大的核心价值）。
