...
核心思路非常清晰：**1. 获取源码定下目录框架** ➡️ **2. 填入庞大的经文数据源** ➡️ **3. 配置 Docker** ➡️ **4. 一键启动** ➡️ **5.（可选）绑定域名**。
...
```yaml
    ...
    environment:
      ...
      # - AI_API_KEY=your_key_here  # 若使用 AI 释义功能可在此配置
```
...
> 💡 `environment` 部分通常**无需修改**。`TZ` 决定容器时区，`AI_API_KEY` 仅在你使用 AI 释义功能时才需要填入（也可在阅读器的设置面板中填写）。
...
你可以随时在容器详情页的**“日志”**控制台中查看构建进度。当你看到日志中出现一连串 `✅` 并最终显示 `Uvicorn running on http://0.0.0.0:8400` 时，刷新网页，属于你个人的私有藏经阁就搭建完毕了。后续因任何原因重启容器，都属“秒开”。

---

## 🌐 域名反向代理与 HTTPS（可选）

如果你不想每次输入 `IP:8400`，可以绑定一个域名（如 `tripitaka.example.com`），通过反向代理转发到本机 `127.0.0.1:8400`。

> 💡 前提：你已在域名服务商处将域名的 A 记录解析到服务器公网 IP。

**1Panel / 宝塔面板用户**（最省心）：

1. 进入面板的 **网站** 菜单，点击 **创建网站 → 反向代理**。
2. **主域名**填你的域名，**代理地址**填 `http://127.0.0.1:8400`。
3. 保存后，在网站设置的 **SSL/证书** 中申请免费的 Let's Encrypt 证书即可开启 HTTPS。

**群晖 NAS 用户**：在 **控制面板 → 登录门户 → 高级 → 反向代理服务器** 中添加一条规则，来源填你的域名和 443 端口，目的地填 `localhost:8400`。

**纯命令行用户**：用 Nginx `proxy_pass` 或 Caddy 反代到 `127.0.0.1:8400` 即可，这里就不展开了。
...
4. 全新镜像启动后会自动读取外部挂载的 `data/user_data`。你的高亮笔记、自定义词典和书签列表都会原封不动地出现在新版本里。
